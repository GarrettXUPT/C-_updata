...
/*
	...
	二、虚函数（调用虚函数，执行的是动态绑定）
		...
		在派生类中加入了override，若是在重写基类中的虚函数，那么只在声明处写override，在具体实现的地方不需要再写override
	...
	五、基类的析构函数一般写成虚函数
		...
		若是调用析构函数，那么先调用的是子类的析构函数，若是父类的析构函数不是虚函数，那么就不会调用父类析构函数，造成内存泄漏风险
		
		普通类可以不设置虚析构函数，虚函数是会增加内存开销的，建立虚函数表
...
void test01() {
	...
	*/
	...
	/*
		这么写的话是可以调用父类的虚函数的，使用作用域运算符可以调用父类中的重载版本
	*/
	...
	//delete phuman;
	delete phuman1;  // 调用了man的析构函数  执行了Human析构函数，先调用子类的析构函数，再调用父类的析构函数
